...
class EmbeddingAnnotator:
    def __init__(
        ...
        self._nodes = ontology_data["nodes"]
    ...
    def _encode_competencies(self) -> Dict[str, torch.Tensor]:
        id_to_node: Dict[str, dict] = {str(n["id"]): n for n in self._nodes}
        texts: List[str] = []
        comp_ids: List[str] = []
    ...
    def retrieve_top_competency_ids(self, text: str, top_k: int) -> List[Tuple[str, float]]:
        if not text or not self._competency_matrix.numel():
            return []
        with torch.inference_mode():
            ...
            device = emb.device
            comp_mat = self._ensure_competency_matrix_on(device)
            scores = emb @ comp_mat.T
            scores = scores[0]
            ...
            for i, idx in enumerate(topi.tolist()):
                if 0 <= idx < len(self._competency_ids):
                    cid = self._competency_ids[idx]
                    out.append((cid, float(topv[i].item())))
            return out
                ...
                if s and c:
                    ...
                    for c0 in range(0, c, chunk_c):
                        ...
                        for j in range(c1 - c0):
                            ...
                            matches = [{"segment_index": idx, "score": score, "segment": segments[idx]} for idx, score
                                       in top_pairs]
