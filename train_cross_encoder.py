"""
Скрипт для обучения cross-encoder на датасетах RusBEIR

Использование:
    python train_cross_encoder.py                    # Начать обучение с нуля
    python train_cross_encoder.py --resume_from models/cross-encoder-rusbeir/checkpoints/epoch_2  # Продолжить обучение
"""
import json
import time
...
CONFIG = {
    ...
    "datasets": [
        ...
        # {
        #     "name": "rus-scibench-retrieval",
        #     "hf_repo": "kngrg/rus-scibench-retrieval",
        #     "hf_repo_qrels": "kngrg/rus-scibench-retrieval-qrels",
        #     "use_train": True,
        #     "use_test": True,
        # },
    ],
    "training": {
        "num_epochs": 3,
        "batch_size": 16,  # Возвращено к исходному значению для лучшего качества
        "learning_rate": 2e-5,
        "warmup_steps": 1000,
        "max_length": 512,  # Возвращено к исходному значению для лучшего качества
        "evaluation_steps": 5000,  # Возвращено к исходному значению
...
def load_rusbeir_dataset(hf_repo: str, hf_repo_qrels: str, split: str = "train"):
    ...
    # Загружаем корпус, запросы и релевантность (RusBEIR использует разные конфиги)
    try:
    ...
    # Релевантность: {query_id: {doc_id: score}}
    for item in qrels_ds:
        ...
        score = item.get("score", 1)  # Binary: 1 = relevant, 0 = not relevant
...
def create_training_examples(
    ...
    """
    Создает примеры для обучения cross-encoder.
    
    Для каждого релевантного (query, document) создаем:
    - 1 positive example (label=1)
    - num_negatives negative examples (label=0) - случайные документы
    """
    examples = []
    ...
    for query_id, relevant_docs in qrels.items():
        ...
        # Берем случайные негативные примеры
        import random
...
def create_evaluation_data(corpus: dict, queries: dict, qrels: dict):
    """
    Создает данные для оценки модели.
    """
    sentence_pairs = []
    labels = []
    ...
    for query_id, relevant_docs in qrels.items():
        ...
        # Для каждого документа создаем пару
        for doc_id in corpus.keys():
            doc_text = corpus[doc_id]["text"]
            sentence_pairs.append([query_text, doc_text])
            
            # Label: 1 если релевантен, 0 если нет
...
def main():
    ...
    for dataset_config in CONFIG["datasets"]:
        ...
        # Загружаем test данные для оценки
        if dataset_config["use_test"]:
            try:
                # Пробуем загрузить test, если нет - используем часть train
                try:
                    ...
                    # Если нет test split, используем часть train данных для валидации
                    ...
                    # Берем только первые 1000 запросов для валидации
    ...
    if args.resume_from:
        ...
        # Загружаем информацию о прогрессе
    ...
    # На RTX 4080 один батч может занимать ~0.1-0.3 секунды
    estimated_time_per_batch = 0.2  # секунды (консервативная оценка)
    ...
    # Обучаем модель с сохранением checkpoints
    print(f"\n{'='*80}")
    ...
    # Сохраняем начальное состояние (только если не продолжаем обучение)
    ...
    try:
        ...
        for epoch in range(start_epoch, CONFIG["training"]["num_epochs"] + 1):
            ...
            # Обучаем одну эпоху
            # Если нет evaluator, отключаем evaluation
            ...
            model.fit(
                ...
                output_path=str(output_dir),  # Сохраняет лучшую модель
                save_best_model=True if evaluator else False,  # Сохраняем лучшую только если есть evaluator
                use_amp=True,  # Используем mixed precision для ускорения
                show_progress_bar=True,  # Показываем прогресс
            )
