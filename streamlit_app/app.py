...
# Streamlit запускает скрипт так, что sys.path[0] указывает на streamlit_app/,
# поэтому добавляем корень проекта, чтобы импортировать локальные пакеты.
...
def _normalize_model_path(value: str) -> str:
    """
    Приводит путь к модели к абсолютному, если это локальный путь в проекте.
    Это нужно, чтобы CrossEncoder не пытался трактовать 'models/...' как HuggingFace repo id.
    """
    raw = (value or "").strip()
    if not raw:
        return raw
    ...
    if p.is_absolute():
        return str(p)
    # Пытаемся интерпретировать как путь относительно корня проекта
    candidate = (PROJECT_DIR / p).resolve()
    if candidate.exists():
        return str(candidate)
    # Иначе оставляем как есть (например, 'namespace/repo_name')
    return raw
...
def _safe_read_uploaded_text(uploaded_file) -> str:
    ...
    if suffix == ".docx":
        try:
            from lib.docx_to_txt import extract_text  # noqa: WPS433 (runtime import is intentional)
...
def _load_full_label_map(ontology_path: str) -> Dict[str, str]:
    """
    Загружает онтологию и строит мапу competency_id -> full_label (или label, если full_label нет).
    Это позволяет в интерфейсе всегда отображать полный путь компетенции.
    """
...
def _build_competencies_table(
    ...
    for i, ann in enumerate(annotations[:top_n], start=1):
        ...
        # Убираем служебные слова "Раздел:" и "Область:" и добавляем переводы строк между уровнями
        comp_name_clean = comp_name.replace("Раздел:", "").replace("Область:", "")
        # Разбиваем по точкам/точкам с пробелом, чтобы разнести уровни по строкам
        ...
        rows.append(
            {
                ...
                # Показываем количество сегментов, учитываемых в doc_score (segment_count),
                # а не только число сегментов, видимых после re-ranking в matches.
...
def main() -> None:
    ...
    # Блок с результатами на всю ширину, чуть ниже формы
    if text is not None and annotations:
        ...
        # Агрегированные разделы и области
        st.divider()
        st.subheader("Агрегированные разделы и области")

        # Загружаем полную онтологию для построения иерархии
        ...
        # Берем top-N листовых компетенций и поднимаем их вклад наверх
        ...
        for ann in annotations[: int(top_n)]:
            ...
            # Для деталей тоже убираем служебные слова и переносим уровни на новые строки
