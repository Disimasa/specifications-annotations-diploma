...
def _clean_json_line(s: str) -> str:
    ...
    if not s:
        return s
    # 1) убираем хвостовую запятую в конце строки
    s = re.sub(r",\s*$", "", s)
    # 2) чиним ключи вида ",evidence": -> "evidence":
...
def _normalize_obj(obj: Dict[str, Any]) -> Dict[str, Any]:
    # Чистим gold_codes от ведущих запятых, пробелов, точек с запятой
    ...
    # Иногда evidence было сохранено под ключом ",evidence" из-за LLM
...
def fix_gold_jsonl(path: Path) -> Tuple[int, int]:
    """
    Возвращает (ok_lines, dropped_lines).
    """
    ok: List[dict] = []
    dropped = 0
    ...
    # Перезаписываем валидный JSONL (один JSON на строку)
    path.write_text(
