...
"""
Генерация test_set (draft) для валидации: документ -> минимально необходимый набор компетенций ГРНТИ.

Требование пользователя: без embedding-retrieval. Поэтому делаем 2-стадийный LLM-процесс:
1) Candidate mining: прогоняем всю листовую часть ГРНТИ чанками и просим LLM выбрать кандидатов
   только из текущего чанка.
2) Final selection: даём LLM объединённый список кандидатов и просим выбрать минимальный набор
   + evidence (номера сегментов/цитаты).

Выход: data/gold/test_set_manual_draft.jsonl

Требуется:
- .env с OPENROUTER_API_KEY
- data/ontology_grnti_with_llm.json (узлы с code/full_label)
"""
...
MAX_SEGMENTS_FOR_LLM = 60     # сколько сегментов текста показываем LLM (после фильтрации)
...
def load_ontology(ontology_path: Path) -> Tuple[Dict[str, dict], Dict[str, str], Dict[str, str]]:
    ...
    - nodes_by_id
    - code_by_id
    - id_by_code (только для тех нод, где code есть)
    """
...
def is_leaf_code(code: str) -> bool:
    # Лист ГРНТИ в нашей 3-уровневой схеме: "XX.YY.ZZ"
    parts = code.split(".")
...
def segment_text(text: str) -> List[str]:
    ...
    # убираем слишком длинные сегменты для LLM (оставим первые 600 символов)
    out: List[str] = []
    for s in segments:
        s = s.strip()
        if not s:
...
def call_openrouter_json(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # вытащим JSON из ответа (на случай если модель добавила мусор)
