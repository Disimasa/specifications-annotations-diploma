...
"""
Оценка качества пайплайна на GOLD-разметке.

Скрипт:
- читает JSONL GOLD (doc_id + gold_codes)
- для каждого документа прогоняет пайплайн аннотации (bi-encoder + опциональный rerank)
- переводит предсказанные competency_id -> code (ГРНТИ) по онтологии
- считает метрики: Precision@K, Recall@K, MRR@K, MAP@K (по кодам ГРНТИ)
...
# Пути
PROJECT_DIR = None  # вычислится автоматически ниже
GOLD_JSONL_PATH = r"data/gold/test_set_manual_draft.jsonl"
ONTOLOGY_PATH = r"data/ontology_grnti_with_llm.json"
PRECOMPUTED_EMBEDDINGS_PATH = r"data/ontology_grnti_embeddings.npz"  # можно оставить, ускоряет прогон
TEXTS_DIR = r"data/specifications/texts"  # fallback если в gold нет text_path

# Параметры пайплайна аннотации
# ВАЖНО: модель bi-encoder должна совпадать с той, под которую считались эмбеддинги онтологии.
# В Streamlit по умолчанию используется deepvk/USER-bge-m3.
BI_ENCODER_MODEL = "deepvk/USER-bge-m3"

THRESHOLD = 0.55
TOP_K = 10  # top-k сегментов на компетенцию (внутри annotate)
MAX_SEGMENT_LENGTH_FOR_CONTEXT = 0
RERANK_TOP_K = 0  # 0 = без cross-encoder
CROSS_ENCODER_MODEL = None  # None -> если RERANK_TOP_K>0, будет взят дефолт из annotator.py
CONFIDENCE_AGGREGATION = "sum"  # sum|max|mean|median|weighted_mean|sum_log_count|mean_log_count
FILTER_SEGMENTS = True

# Метрики
EVAL_KS = (1, 3, 5, 10, 20)
# Сколько предсказаний (кодов) максимум собирать из ранжированного списка.
# По умолчанию достаточно max(EVAL_KS), иначе можно случайно насобирать сотни/тысячи кодов при низком THRESHOLD.
...
def _load_ontology_code_map(ontology_path: Path) -> Dict[str, str]:
    """
    competency_id -> code (строка вида XX.YY.ZZ или др.)
    """
...
def _to_level_code(code: str, level: int) -> Optional[str]:
    """
    Приводит код ГРНТИ XX[.YY[.ZZ]] к уровню:
    level=3 -> XX.YY.ZZ (лист)
    level=2 -> XX.YY     (parent)
    level=1 -> XX        (grandparent)
    """
    parts = code.split(".")
...
def _aggregate_codes_to_level(codes: Sequence[str], level: int) -> List[str]:
    """
    Делает ранжированный список кодов нужного уровня:
    - сохраняет порядок по предсказаниям
    - удаляет дубликаты
    """
    seen: set[str] = set()
    out: List[str] = []
...
def _read_gold_items(gold_path: Path, texts_dir: Path) -> List[GoldItem]:
    ...
    for raw_line in gold_path.read_text(encoding="utf-8").splitlines():
        ...
        if isinstance(gold_codes_raw, list):
            # ожидаем список строк (как в нашем генераторе)
...
def recall_at_k(pred: Sequence[str], gold: Sequence[str], k: int) -> float:
    ...
    g = set(gold)
    if not g:
        return 1.0  # если в gold пусто, считаем recall=1 (ничего не требовалось)
    pred_k = pred[:k]
...
def main() -> None:
    ...
    # Печатаем основные метрики в консоль
    for k in EVAL_KS:
