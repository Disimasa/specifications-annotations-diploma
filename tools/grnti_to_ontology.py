...
"""
Скрипт для скачивания и конвертации рубрикатора ГРНТИ (https://grnti.ru/)
в формат онтологии, совместимый с текущей моделью (nodes + links).

Результат: data/ontology_grnti.json
"""
...
# Совместимость с существующей онтологией
...
@dataclass
class GrntiNode:
    code: str           # например, "20", "20.23", "20.23.29"
    title: str          # текст без кода, например "ИНФОРМАТИКА"
    level: int          # 1, 2 или 3
    ...
    def node_id(self) -> str:
        # Встраиваем код в id, чтобы он однозначно идентифицировался
...
def fetch_html(url: str, max_retries: int = 5, timeout: int = 30, base_sleep: float = 1.0) -> str:
    """
    Аккуратный HTTP-запрос с повторными попытками и небольшими паузами.
    При желании можно задать прокси через переменную окружения GRNTI_PROXY.
    """
...
def parse_top_level() -> List[GrntiNode]:
    """
    Парсит главную страницу https://grnti.ru/ и возвращает список разделов вида:
    20: ИНФОРМАТИКА, 50: АВТОМАТИКА. ВЫЧИСЛИТЕЛЬНАЯ ТЕХНИКА и т.д.
    """
    ...
    for params, text in links:
        if "p1" not in params or len(params) != 1:
            continue
        code = params["p1"]
        # Текст на главной обычно имеет вид "20 ИНФОРМАТИКА"
        # Попробуем отделить код от названия по первому пробелу
        title = text
...
def parse_second_level(p1: str) -> List[GrntiNode]:
    """
    Для кода 20 возвращает список узлов вида 20.23, 20.51 и т.д.
    """
    ...
    for params, text in links:
        ...
        p2 = params["p2"]
        code = f"{p1}.{p2}"
        # Текст ссылки обычно "20.23 Информационный поиск"
        title = text
        if text.startswith(code):
            title = text[len(code) :].strip(" :–-")
...
def parse_third_level(p1: str, p2: str) -> List[GrntiNode]:
    """
    Для пары (20, 23) возвращает узлы вида 20.23.29 и т.п.
    """
    ...
    for params, text in links:
        ...
        p3 = params["p3"]
        code = f"{p1}.{p2}.{p3}"
        # Текст ссылки обычно "20.23.29 Электронные библиотеки"
        if not text.startswith(prefix):
            # иногда на странице могут быть ссылки на другие уровни — фильтруем
            continue
        title = text[len(code) :].strip(" :–-")
...
def build_ontology() -> Dict[str, list]:
    """
    Обходит все уровни ГРНТИ и строит JSON в формате:
    ...
    # Добавляем корневой узел для ГРНТИ
    ...
    for sec in tqdm(top_level, desc="p1 (верхний уровень)", unit="раздел"):
        ...
        for sub in tqdm(subnodes, desc=f"p2 для {p1}", leave=False, unit="подраздел"):
            ...
            for spec in leaf_nodes:
                # В ГРНТИ листовые коды могут иногда дублироваться — защищаемся
